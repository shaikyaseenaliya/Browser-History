...
class App extends Component {
  state = {
    searchInput: '',
    latesHistoryList: initialHistoryList,
    isTrue: false,
  }

  FillFunction = value => {
    const {latesHistoryList} = this.state
    const newHistoryList = latesHistoryList.filter(
      eachValue => eachValue.id !== value,
    )

    if (newHistoryList.length === 0) {
      this.setstate({latesHistoryList: newHistoryList, isTrue: true})
    } else {
      this.setState({latesHistoryList: newHistoryList})
    }
  }
  ...
  render() {
    const {searchInput, latesHistoryList} = this.state
    let {isTrue} = this.state
    const newHistoryList = latesHistoryList.filter(eachValue =>
